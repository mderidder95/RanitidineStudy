...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (997276,953076,43009003,21600086,950696,961047,19011685)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (997276,953076,43009003,21600086,950696,961047,19011685)
...
WHERE YEAR(C.drug_exposure_start_date) - P.year_of_birth >= 0
...
) A on A.person_id = P.person_id  AND A.START_DATE >= DATEADD(day,-365,P.START_DATE) AND A.START_DATE <= DATEADD(day,-1,P.START_DATE)
